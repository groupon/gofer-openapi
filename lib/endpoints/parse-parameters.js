...
function parseParameters(parameters, components) {
    ...
    if (optTypeProps.length === 1 && hasBody) {
        ...
        fetchOpts = Object.entries(params).flatMap(([opt, vars]) => vars.length > 0
            ? [
                t.objectProperty(t.identifier(opt), t.objectExpression(vars.map(([name, camelName]) => t.objectProperty(t.identifier(name), t.memberExpression(t.identifier('opts'), t.identifier(camelName)))))),
            ]
            : []);
        if (hasBody) {

...
function goferFromOpenAPI(openAPI, { className, extendsClassName = 'Gofer', format, target }) {
    ...
    const goferImports = extendsClassName === 'Gofer'
        ? [
            t.importDeclaration([t.importDefaultSpecifier(t.identifier('Gofer'))], t.stringLiteral('gofer')),
        ]
        : [];
    const typeDecls = (0, type_decls_1.default)(spec.components);
    const endpoints = (0, endpoints_1.default)(spec);
    const klass = t.exportNamedDeclaration(t.classDeclaration(t.identifier(className), t.identifier(extendsClassName), t.classBody(endpoints)));
    const bareTS = (0, generator_1.default)(t.program([...goferImports, ...typeDecls, klass])).code;
